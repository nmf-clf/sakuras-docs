...
前端工程化即，在企业级的前端项目开发中，把前端开发所需的工具、技术、流程、经验等进行规范化、标准化。这样有利于前端开发自成体系，有一套<span color=red>标准的开发方案和流程</span>。
...
- 它提供了<span color=red>前端模块化开发</span>支持，以及<span color=red>代码压缩混淆、处理浏览器端 JavaScript 的兼容性、性能优化</span>等功能
