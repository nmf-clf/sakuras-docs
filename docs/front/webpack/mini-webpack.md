...
因此我们要实现一个自己的打包工具 `skpack`，就需要满足以下：
...
2. 生成 `package.json` 文件 - 执行`npm init -y`
    ...
    ```
3. 生成命令 `npm link`，`npm link` 会在全局目录下生成一个 `skpack` 命令（创建全局快捷方式），你可以在全局 `npm` 包的 `node_modules` 目录下找到 `skpack` 的快捷方式，它指向了你的项目目录下的 `bin/index.js` 文件
...
> `loader` 是一个函数，接收一个参数，返回一个转换后的结果。
...
以下是部分 `loader` 的使用场景：
- 由于浏览器不支持 `less`，因此需要使用 `less-loader` 将 `less` 转换成 `css`，浏览器才能识别 
- 由于需要将转换后的 `css` 插入到 `html` 中，因此需要使用 `style-loader` 将 `css` 转换成 `style` 标签插入到 `html` 中
- 由于浏览器不支持 `ES6` 语法，因此需要使用 `babel-loader` 将 `ES6` 转换成 `ES5`，浏览器才能识别
- 由于浏览器不支持 `vue` 语法，因此需要使用 `vue-loader` 将 `vue` 转换成 `js`，浏览器才能识别
...
在 `webpack` 源码中，`use` 数组中的 `loader` 是从后往前执行的，即：`less-loader` -> `css-loader` -> `style-loader`;  
每个 `loader` 都会将转换后的结果传递给下一个 `loader`;  
